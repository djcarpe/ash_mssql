defmodule AshMssql.Mix.Helpers do
  @moduledoc false
  ...
  def repos!(opts, args) do
    ...
    resources =
      ...
      |> Enum.filter(&(Ash.DataLayer.data_layer(&1) == AshMssql.DataLayer))
      |> case do
        [] ->
          raise """
          No resources with `data_layer: AshMssql.DataLayer` found in the domains #{Enum.map_join(domains, ",", &inspect/1)}.

          Must be able to find at least one resource with `data_layer: AshMssql.DataLayer`.
          """
    ...
    resources
    |> Enum.map(&AshMssql.DataLayer.Info.repo(&1))
    |> Enum.uniq()
    |> case do
      [] ->
        raise """
        ...
        The following resources were found with `data_layer: AshMssql.DataLayer`:
