if Code.ensure_loaded?(Igniter) do
  defmodule Mix.Tasks.AshMssql.Install do
    @moduledoc "Installs AshMssql. Should be run with `mix igniter.install ash_mssql`"
    @shortdoc @moduledoc
    ...
    def igniter(igniter) do
      ...
      igniter
      |> Igniter.Project.Formatter.import_dep(:ash_mssql)
      |> setup_aliases()
    ...
    defp configure_test(igniter, otp_app, repo) do
      if Igniter.Project.Config.configures_key?(igniter, "test.exs", otp_app, [repo]) do
        ...
        |> Igniter.Project.Config.configure_new("test.exs", otp_app, [repo, :port], 1433)
        |> Igniter.Project.Config.configure_new(
          "test.exs",
          otp_app,
          [repo, :hostname],
    ...
    defp configure_dev(igniter, otp_app, repo) do
      if Igniter.Project.Config.configures_key?(igniter, "dev.exs", otp_app, [repo]) do
        ...
        |> Igniter.Project.Config.configure_new("dev.exs", otp_app, [repo, :port], 1433)
        |> Igniter.Project.Config.configure_new(
          "dev.exs",
          otp_app,
    ...
    defp setup_repo_module(igniter, otp_app, repo) do
      default_repo_contents =
        """
        use AshMssql.Repo, otp_app: #{inspect(otp_app)}
        """
        ...
        fn zipper ->
          case Igniter.Code.Module.move_to_use(zipper, Ecto.Repo) do
            {:ok, _} ->
              ...
              |> use_ash_mssql_instead_of_ecto()
              |> Sourceror.Zipper.top()
              |> remove_adapter_option()
              |> then(&{:ok, &1})

            _ ->
              case Igniter.Code.Module.move_to_use(zipper, AshMssql.Repo) do
                {:ok, _} ->
                  {:ok, zipper}
                ...
                _ ->
                  {:error,
                   """
                   Repo module #{inspect(repo)} existed, but was not an `Ecto.Repo` or an `AshMssql.Repo`.

                   Please rerun the ash_mssql installer with the `--repo` option to specify a repo.
                   """}
              end
          end
        end
    ...
    defp use_ash_mssql_instead_of_ecto(zipper) do
      with {:ok, zipper} <- Igniter.Code.Module.move_to_module_using(zipper, Ecto.Repo),
           ...
           {:ok, zipper} <-
             Igniter.Code.Function.update_nth_argument(zipper, 0, fn zipper ->
               {:ok, Igniter.Code.Common.replace_code(zipper, AshMssql.Repo)}
             end) do
        zipper
      else
        _ ->
    ...
    defp remove_adapter_option(zipper) do
      with {:ok, zipper} <- Igniter.Code.Module.move_to_module_using(zipper, AshMssql.Repo),
           {:ok, zipper} <- Igniter.Code.Module.move_to_use(zipper, AshMssql.Repo),
           {:ok, zipper} <-
  ...
  defmodule Mix.Tasks.AshMssql.Install do
    ...
    def run(_argv) do
      Mix.shell().error("""
      The task 'ash_mssql.install' requires igniter to be run.
