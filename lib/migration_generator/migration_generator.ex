defmodule AshMssql.MigrationGenerator do
  @moduledoc false
  ...
  def generate(domains, opts \\ []) do
    ...
    snapshots =
      ...
      |> Enum.filter(fn resource ->
        Ash.DataLayer.data_layer(resource) == AshMssql.DataLayer &&
          AshMssql.DataLayer.Info.migrate?(resource)
      end)
  ...
  def take_snapshots(domain, repo, only_resources \\ nil) do
    ...
    |> Enum.filter(fn resource ->
      Ash.DataLayer.data_layer(resource) == AshMssql.DataLayer &&
        AshMssql.DataLayer.Info.repo(resource) == repo &&
  ...
  defp add_custom_index_name(custom_index, table) do
    ...
    |> Map.put_new_lazy(:name, fn ->
      AshMssql.CustomIndex.name(table, %{fields: custom_index.fields})
    end)
    |> Map.update!(
      :name,
      &(&1 || AshMssql.CustomIndex.name(table, %{fields: custom_index.fields}))
    )
  end
  ...
  def get_snapshots(resource, all_resources) do
    Code.ensure_compiled!(AshMssql.DataLayer.Info.repo(resource))
    ...
    if AshMssql.DataLayer.Info.polymorphic?(resource) do
      ...
      |> Enum.map(fn relationship ->
        ...
        |> Map.update!(:identities, fn identities ->
          identity_index_names = AshMssql.DataLayer.Info.identity_index_names(resource)
              ...
              Map.put(attribute, :references, %{
                ...
                destination_attribute_default:
                  default(
                    source_attribute,
                    relationship.destination,
                    AshMssql.DataLayer.Info.repo(relationship.destination)
                  ),
                deferrable: false,
                destination_attribute_generated: source_attribute.generated?,
                multitenancy: multitenancy(relationship.source),
                table: AshMssql.DataLayer.Info.table(relationship.source),
                on_delete: AshMssql.DataLayer.Info.polymorphic_on_delete(relationship.source),
                on_update: AshMssql.DataLayer.Info.polymorphic_on_update(relationship.source),
                primary_key?: source_attribute.primary_key?,
                name:
                  AshMssql.DataLayer.Info.polymorphic_name(relationship.source) ||
      ...
      [do_snapshot(resource, AshMssql.DataLayer.Info.table(resource))]
    end
  end
  ...
  defp do_snapshot(resource, table) do
    snapshot = %{
      ...
      table: table || AshMssql.DataLayer.Info.table(resource),
      ...
      base_filter: AshMssql.DataLayer.Info.base_filter_sql(resource),
  ...
  defp custom_indexes(resource) do
    ...
    |> Enum.map(fn custom_index ->
      Map.take(custom_index, AshMssql.CustomIndex.fields())
    end)
  end

  defp custom_statements(resource) do
    resource
    |> AshMssql.DataLayer.Info.custom_statements()
    |> Enum.map(fn custom_statement ->
      Map.take(custom_statement, AshMssql.Statement.fields())
    end)
  end
  ...
  defp attributes(resource, table) do
    repo = AshMssql.DataLayer.Info.repo(resource)
    ignored = AshMssql.DataLayer.Info.migration_ignore_attributes(resource) || []
          ...
          %{
            ...
            table:
              relationship.context[:data_layer][:table] ||
                AshMssql.DataLayer.Info.table(relationship.destination)
          }
        end
      end
    end)
  ...
  defp foreign_key?(relationship) do
    Ash.DataLayer.data_layer(relationship.source) == AshMssql.DataLayer &&
      AshMssql.DataLayer.Info.repo(relationship.source) ==
        AshMssql.DataLayer.Info.repo(relationship.destination)
  end
  ...
  defp identities(resource) do
    ...
    |> case do
      ...
      identities ->
        ...
        if base_filter && !AshMssql.DataLayer.Info.base_filter_sql(resource) do
          raise """
    ...
    |> Enum.reject(fn identity ->
      identity.name in AshMssql.DataLayer.Info.skip_unique_indexes(resource)
    end)
    ...
    |> Enum.map(fn identity ->
      Map.put(
        ...
        :index_name,
        identity_index_names[identity.name] ||
          "#{AshMssql.DataLayer.Info.table(resource)}_#{identity.name}_index"
      )
    end)
