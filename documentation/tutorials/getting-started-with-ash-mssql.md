# Getting Started With AshMssql

## Goals

In this guide we will:

1. Setup AshMssql, which includes setting up [Ecto](https://hexdocs.pm/ecto/Ecto.html)
2. Add AshMssql to the resources created in [the Ash getting started guide](https://hexdocs.pm/ash/get-started.html)
3. Show how the various features of AshMssql can help you work quickly and cleanly against a mssql database
4. Highlight some of the more advanced features you can use when using AshMssql.
5. Point you to additional resources you may need on your journey

## Requirements

- A working MSSQL installation, with a sufficiently permissive user
...
Add the `:ash_mssql` dependency to your application

`{:ash_mssql, "~> 0.1.2"}`

Add `:ash_mssql` to your `.formatter.exs` file

```elixir
[
  # import the formatter rules from `:ash_mssql`
  import_deps: [..., :ash_mssql],
  inputs: [...]
]
```
...
Create `lib/helpdesk/repo.ex` with the following contents. `AshMssql.Repo` is a thin wrapper around `Ecto.Repo`, so see their documentation for how to use it if you need to use it directly. For standard Ash usage, all you will need to do is configure your resources to use your repo.

```elixir
# in lib/helpdesk/repo.ex

defmodule Helpdesk.Repo do
  use AshMssql.Repo, otp_app: :helpdesk
end
```
...
### Add AshMssql to our resources

Now we can add the data layer to our resources. The basic configuration for a resource requires the `d:AshMssql.mssql|table` and the `d:AshMssql.mssql|repo`.
...
First, we'll create the database with `mix ash_mssql.create`.

Then we will generate database migrations. This is one of the many ways that AshMssql can save time and reduce complexity.

```bash
mix ash_mssql.generate_migrations --name add_tickets_and_representatives
```
...
Typically, you need to write these by hand. AshMssql, however, will store snapshots each time you run the command to generate migrations and will figure out what migrations need to be created.
...
And now we can read that data. You should see some debug logs that show the sql queries AshMssql is generating.
...
And, naturally, now that we are storing this in mssql, this database is persisted even if we stop/start our application. The nice thing, however, is that this was the _exact_ same code that we ran against our resources when they were backed by ETS.
...
- Check out the data layer docs: `AshMssql.DataLayer`

- [Ecto's documentation](https://hexdocs.pm/ecto/Ecto.html). AshMssql (and much of Ash itself) is made possible by the amazing Ecto. If you find yourself looking for escape hatches when using Ash or ways to work directly with your database, you will want to know how Ecto works. Ash and AshMssql intentionally do not hide Ecto, and in fact encourages its use whenever you need an escape hatch.

- [Ecto's Migration documentation](https://hexdocs.pm/ecto_sql/Ecto.Migration.html) read more about migrations. Even with the ash_mssql migration generator, you will very likely need to modify your own migrations some day.
