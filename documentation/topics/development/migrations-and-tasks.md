...
Ash comes with its own tasks, and AshMssql exposes lower level tasks that you can use if necessary. This guide shows the process using `ash.*` tasks, and the `ash_mssql.*` tasks are illustrated at the bottom.
...
For more information on generating migrations, run `mix help ash_mssql.generate_migrations` (the underlying task that is called by `mix ash.migrate`)
...
mix ash_mssql.rollback -n $N_MIGRATIONS
...
# AshMssql-specific tasks

- `mix ash_mssql.generate_migrations`
- `mix ash_mssql.create`
- `mix ash_mssql.migrate`
- `mix ash_mssql.rollback`
- `mix ash_mssql.drop`
