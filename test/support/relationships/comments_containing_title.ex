defmodule AshMssql.Test.Post.CommentsContainingTitle do
  @moduledoc false

  use Ash.Resource.ManualRelationship
  use AshMssql.ManualRelationship
  require Ash.Query
  require Ecto.Query
  ...
  def ash_mssql_join(query, _opts, current_binding, as_binding, :inner, destination_query) do
    {:ok,
  ...
  def ash_mssql_join(query, _opts, current_binding, as_binding, :left, destination_query) do
    {:ok,
     Ecto.Query.from(_ in query,
  ...
  def ash_mssql_subquery(_opts, current_binding, as_binding, destination_query) do
    {:ok,
     Ecto.Query.from(_ in destination_query,
       where: parent_as(^current_binding).id == as(^as_binding).post_id,
       where:
         fragment("CHARINDEX(?, ?) > 0", parent_as(^current_binding).title, as(^as_binding).title)
     )}
  end
end
