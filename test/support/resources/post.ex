defmodule AshMssql.Test.Post do
  @moduledoc false
  use Ash.Resource,
  ...
  attributes do
    ...
    attribute(:status, AshMssql.Test.Types.Status, public?: true)
    attribute(:status_enum, AshMssql.Test.Types.StatusEnum, public?: true)

    attribute(:status_enum_no_cast, AshMssql.Test.Types.StatusEnumNoCast,
      source: :status_enum,
      public?: true
    )
  ...
  relationships do
    belongs_to :organization, AshMssql.Test.Organization do
      public?(true)
    ...
    has_many(:comments, AshMssql.Test.Comment, destination_attribute: :post_id, public?: true)

    has_many :comments_matching_post_title, AshMssql.Test.Comment do
      public?(true)
      filter(expr(title == parent_expr(title)))
    end

    has_many :popular_comments, AshMssql.Test.Comment do
      public?(true)
      destination_attribute(:post_id)
      filter(expr(likes > 10))
    end

    has_many :comments_containing_title, AshMssql.Test.Comment do
      public?(true)
      manual(AshMssql.Test.Post.CommentsContainingTitle)
    end

    has_many(:ratings, AshMssql.Test.Rating,
      public?: true,
      destination_attribute: :resource_id,
      relationship_context: %{data_layer: %{table: "post_ratings"}}
    )

    has_many(:post_links, AshMssql.Test.PostLink,
      public?: true,
    ...
    many_to_many(:linked_posts, __MODULE__,
      public?: true,
      through: AshMssql.Test.PostLink,
      join_relationship: :post_links,
      source_attribute_on_join_resource: :source_post_id,
      destination_attribute_on_join_resource: :destination_post_id
    )

    has_many(:views, AshMssql.Test.PostView, public?: true)
  end
  ...
  calculations do
    ...
    calculate(
      ...
      expr(
        ...
        if(
          fragment("(? <= DATEADD(month, 1, ?))", now(), created_at),
          true,
          false
        )
      )
